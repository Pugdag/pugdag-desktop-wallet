...
//TODO
window.PWA_MODULES={};
window.PWA_MODULES["@pugdag/wallet-pwa"] = "N/A";
window.PWA_MODULES["@aspectron/flow-ux"] = "N/A";
window.PWA_MODULES["@pugdag/ux"] = "N/A";
window.PWA_MODULES["@pugdag/grpc-web"] = "N/A";
window.PWA_MODULES["@pugdag/wallet"] = "N/A";
window.PWA_MODULES["@pugdag/grpc"] = "N/A";
window.PWA_MODULES["@pugdag/core-lib"] = "N/A";

class PugdagDesktopApp extends FlowApp{
	render(){
		let walletMeta = {"generator":"PugDAG Desktop"}
		let list = [
			['PugDAG','MIT','Copyright (c) 2024 PugDAG Developers'],
...
//			['Mosquitto','EDL-V10 EPL-V10','Copyright (c) 2007, Eclipse Foundation, Inc. and its licensors'],
		...
		let copyright = [
			['PugDAG Desktop','Copyright (c) 2024 PugDAG Developers', 'All Rights Reserved'],
		...
		return html`
		<flow-caption-bar
			logo="/resources/images/pugdag-logo-light-bg.png">PugDAG Desktop</flow-caption-bar>
		<tab-content for="home">
			...
			<div id='pugdag-resources'>

				<flow-expandable>
					<div slot="title" is="i18n-div" caption>PUGDAG RESOURCES</div>
			...
			<flow-expandable class="donation-addresses" no-info _icon="fal:donate">
				...
				<p is="i18n-p">
					If you wish to further the development of the pugdag ecosystem, we accept pugdag donations at the following addresses:
				</p>
				${
		...
		<tab-content for="settings">
			<flow-form-control icon="fal:database">
				...
				<p slot="info" is="i18n-p">
					Data Folder location is used for storage by all PugDAG Desktop
					modules. In default configuration this includes Pugdagd blockchain
			...
			<flow-form-control icon="fal:tools">
				...
				<p slot="info" is="i18n-p">
					Advanced mode allows you to manually configure, interact with and monitor PugDAG Desktop services.
				</p>
			</flow-form-control>
			<flow-form-control icon="fal:cog" class="advanced-tool">
				...
				<p slot="info" is="i18n-p">
					When enabled, PugDAG Desktop runs itself and it's services hidden in the background
			...
			<flow-form-control icon="fal:cog" class="advanced-tool">
				...
				<p slot="info" is="i18n-p">When Autocompound enabled, PugDAG Desktop compound UTXOs when there will be compoundable count of unspent transaction outputs (UTXOs).'Use latest Change address' will compund UTXOs using current/latest change address instead of first change address. </p>
			</flow-form-control>
			<!-- flow-form-control icon="fal:drafting-compass" class="advanced-tool">
				...
				<p slot="info" is="i18n-p">
					You can stream PugDAG Desktop metrics to your own StatsD-compatible server.
				</p>
			</flow-form-control-->
			...
			<flow-form-control id="settings-script" icon="fal:tasks">
				...
				<p slot="info" is="i18n-p">
					Configuration editor allows you to customize PugDAG Desktop environment.
					PugDAG Desktop configuration is represented in JSON
			...
			<flow-form-control id="settings-templates" icon="fal:file-alt"
				...
				<p slot="info" is="i18n-p">
					Configuration templates allow you to load pre-made PugDAG Desktop configurations.
				</p>
			</flow-form-control>
			<flow-form-control icon="fal:database">
				...
				<p slot="info" is="i18n-p">
					It will delete datadir (Data Folder) and restart pugdagd node to re-sync
				</p>
			</flow-form-control>
			<flow-form-control icon="fal:database">
				...
				<p slot="info" is="i18n-p">
					Start the pugdagd node without enabling the '--utxoindex' parameter until the synchronization is complete, re-enable this parameter. (Can be used to fix inaccurate balance display)
				</p>
			</flow-form-control>
			<div style="height:192px;"></div>
		</tab-content>
		<tab-content for="wallet" class="wallet" data-active-display="flex">
			<pugdag-wallet .walletMeta='${walletMeta}' hideNetwork hidefaucet 
				_hideQRScanner hideopenwalletlogo></pugdag-wallet>
		</tab-content>
...
//		this.checkForUpdates().then();
	...
	initRPC(){
		let rpc = new FlowRPC({bcastChannel:'pugdag-desktop'});
		this.rpc = rpc;
	...
	async initManager(){
		...
		manager.on("task-start", (daemon)=>{
			...
			this.refreshApps();
			const {wallet} = this;
			if(!wallet || daemon.task.type!='pugdagd' || !this.rpcDisconnect){
				return
			}
		...
		manager.on("before-interrupt", ({daemon, interrupt})=>{
			...
			const {wallet} = this;
			if(!wallet || daemon.task.type!='pugdagd')
				return
		...
		manager.on('sync-status', (data) => {
			//console.log("sync-status:data", data);
			let wallet = this.wallet || this.qS('pugdag-wallet');
	...
	async initWallet() {
		let wallet = this.qS('pugdag-wallet');
		this.wallet = wallet;
		this.applyCompoundConfig();
		let settings = await this.get_default_local_pugdagd_settings();
	...
	async get_default_local_pugdagd_settings() {
		
		let {config:daemons} = await this.get("get-modules-config");
		console.log("############### DAEMONS", daemons);
		let pugdagd = Object.entries(daemons).map(([k,v]) => { 
			const { args } = v;
	...
	setTheme(theme){
		...
		if(this.caption)
			this.caption.logo = `/resources/images/pugdag-logo-${theme}-bg.png`
		...
		document.body.dispatchEvent(new CustomEvent("flow-theme-changed"));
		this.querySelector("pugdag-wallet")?.requestUpdate("theme", null)
	}
	initCaption(){
		...
		this.caption.logo = `/resources/images/pugdag-logo-${this.theme||"light"}-bg.png`;
	...
	async initSettings(){
		...
		if(statsdPrefixInput)
			this.statsdPrefix = statsdPrefixInput.value = config.statsdPrefix || "pugdag-desktop.$HOSTNAME";
	...
	initReleaseNotes(){
		...
		dialog.content = 
`#	Welcome to PugDAG Desktop ${pkg.version}!

Useful resources:
- Pugdag Discord: https://discord.gg/Swj4saMdUV
- Pugdag GitHub: https://github.com/Pugdag/
- PugDAG Desktop GitHub: https://github.com/Pugdag/pugdag-desktop
...
${changelogContent}`;
	...
	async initDaemons(daemons){
		...
		let {params} = this.getModuleArgs('pugdagd:', daemons);
	...
	initWin(){
		...
		this.closeWin = async(isExit)=>{
			...
			if(isExit !== true && !this.runInBG){
				let {btn} = await FlowDialog.show({
					title:i18n.t("EXIT PugDAG Desktop"),
					body:i18n.t("Are you sure?"),
					btns:[{
						text:i18n.t('Cancel'),
						value:'cancel'
	...
	showApps(daemons) {
		...
		let entries = Object.values(appsMap).map((app) => {
			...
			if(!location && app.engines?.pugdag-desktop) {
			...
			return `${this.debugAppLinks?JSON.stringify(app)+"::"+location:""}
				<flow-window-link
					...
					width="${width}"
					height="${height}"
					icon="resources/images/pugdag-desktop-icon.png"
					resizable
					frame
	...
	async checkForUpdates(force = false) {
		...
		const url = 'https://pugdag.com/pugdag-desktop/version.json';
		...
		if(!data) {
			console.log("missing version data in https://pugdag.com/pugdag-desktop/version.json");
			return false;
		}
		...
		if(is_gt) {
			...
			let {btn} = await FlowDialog.show({
				title:i18n.t("PugDAG Desktop Update"),
			...
			if(btn == 'ok') {
				require('nw.gui').Shell.openExternal('https://pugdag.com');

				let confirm = await FlowDialog.show({
					title:i18n.t("PugDAG Desktop Update"),
					body:html`Please download the latest version from  <flow-link href="https://pugdag.com" target="_blank">https://pugdag.com</flow-link>
					<br/>&nbsp;<br/>
					PugDAG Desktop will now shutdown`,
					btns:[{
						text:i18n.t('Cancel'),
						value:'cancel'
					},{
...
PugdagDesktopApp.define("pugdag-desktop-app")
