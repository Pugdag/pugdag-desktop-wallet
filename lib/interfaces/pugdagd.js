...
const {Client:PugdagGRPC} = require('@pugdag/grpc-node');
const pkg = require("../../package");
const natUpnp = require("@runonflux/nat-upnp");

module.exports = class Pugdagd extends Daemon {
	...
	start() {
		this.log("pugdagd::start() invoked...");
		return new Promise(async (resolve,reject) => {

			if(this.task.conf['reset-peers'])
				this.cleanupResidualData();

			this.log("pugdagd::start() running");
			...
			if (!("uacomment" in args)) {
				const uacomment = `pugdag-desktop_${pkg['version']}`
				this.writeToSink(`PUGDAGD: Adding uacomment=${uacomment} to arguments\n`);
			...
			if (this.task.conf['upnpEnabled'] && args['listen'] && args['listen'].indexOf(":") !== -1) {
				...
				if (externalip) {
					if (!("externalip" in args)) {
						this.writeToSink(`PUGDAGD: Adding externalip=${externalip} to arguments\n`);
					...
					if (publicPort) {
						...
						this.writeToSink(`PUGDAGD: Changing externalip from ${args['externalip']} to ${externalip}:${publicPort} in arguments\n`);
						args['externalip'] = `${externalip}:${publicPort}`;
						if (publicPort !== nodePort) {
							this.writeToSink(`PUGDAGD: Changing listener port from ${nodePort} to ${publicPort} in arguments\n`);
							args['listen'] = `${args['listen'].split(':')[0]}:${publicPort}`;
						}
						if (!("maxinpeers" in args)) {
							this.writeToSink(`PUGDAGD: Adding maxinpeers=${this.upnpMaxInPeers} to arguments\n`);
			...
			const configFile = path.join(this.folder, 'pugdagd.conf');
			...
			await this.VSARGS([this.getDeployableBinary('pugdagd'),'--version']);

			this.run({
				args : async () => {
					let _args_ = [
 						this.getDeployableBinary('pugdagd')
					];
			...
			resolve();
			// mutes all pugdagd processes by default
			this.mute = true;
		})
	}
	...
	async stop() {
		...
		if(this.rpc) {
			...
			try {
				if(!process.env.PUGDAG_JSON_RPC)				
		...
		return new Promise(async (resolve, reject) => {
	        ...
	        try {
				...
				this.data = this.sourcesToData(sources);
	             ...
	             console.log('PUGDAGD monitor:'.bold,ex);
	            return reject(ex);
	        }
