...
class PugdagProcessManager extends EventEmitter{
	...
	async start(daemons={}) {
		...
		const startupTypeOrder = ['pugdagd','mqtt','mysql','pgsql', 'txgen','app'];
		const typeMap = { };
	...
	async stop() {
		//const shutdownTypeOrder = ['pugdagd','mqtt','mysql','pgsql','simulator','txgen'];
		//const typeMap = { };
	...
	getDeployableBinary(...args) {
		...
		switch(buildType) {
			...
			case 'local': {
				return path.join(os.homedir(),'.pugdag-desktop','bin',utils.platform, ...args)+this.PLATFORM_BINARY_EXTENSION;
			}			
		}
	}
	...
	enumerateApps() {
		...
		return packages.map(({ folder, pkg }) => {
			let { name, description, main, engines, pugdag } = pkg;
			return {
				folder, name, description, main, engines, ...pugdag
			}
		});
	}
...
module.exports = PugdagProcessManager;
