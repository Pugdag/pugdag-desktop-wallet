...
RUN addgroup -S pugdag-desktop && adduser -S pugdag-desktop -G pugdag-desktop
RUN mkdir -p /run/postgresql
RUN chown pugdag-desktop:pugdag-desktop /run/postgresql

# Tell docker that all future commands should run as the appuser user

WORKDIR /home/pugdag-desktop/releases/pugdag-desktop
COPY . .
RUN rm -rf node_modules
RUN rm package-lock.json
#RUN chown -R pugdag-desktop:pugdag-desktop .


RUN npm install
RUN npm install @pugdag/process-list
...
ENTRYPOINT ["node","pugdag-desktop.js","--init","--nice","--mine"]
